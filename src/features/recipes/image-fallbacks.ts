...
const SPECIFIC_RECIPE_IMAGES: Record<string, string> = {
  "abacate toast":
...
const KEYWORD_IMAGES: Array<{ terms: string[]; image: string }> = [
  ...
  {
    terms: ["macarrao", "massa", "pasta", "lasanha", "talharim"],
...
export function getRecipeImageFallback(recipe: Pick<Recipe, "title" | "category" | "ingredients">): string {
  ...
  const specificImage = SPECIFIC_RECIPE_IMAGES[title];
